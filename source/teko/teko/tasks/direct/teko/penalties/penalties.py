...
# ------------------------------------------
# Note: Currently these are NOT imported by reward_functions.py
...
def _safe_norm(x, dim=-1):
    """Safe norm computation that handles NaN/Inf values."""
...
def compute_velocity_penalty_when_close(env, surface_xy):
    """Penalize high velocity when close to goal."""
...
def compute_oscillation_penalty(env):
    """Penalize rapid velocity changes (jerky motion)."""
...
def compute_robot_collision_penalty(env, surface_xy):
    """Penalty for being too close but not successfully docked."""
